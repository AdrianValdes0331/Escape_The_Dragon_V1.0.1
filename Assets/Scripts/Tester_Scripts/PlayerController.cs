...
namespace TarodevController {
    ...
    public class PlayerController : MonoBehaviour, IPlayerController {
        ...
        private float Horizontal_Change;
        ...
        void Update() {
            ...
            Horizontal_Change = UnityEngine.Input.GetAxisRaw("Horizontal");
            if (Horizontal_Change < 0.0f)
            {
                transform.localScale = new Vector3(-1.0f, 1.0f, 1.0f);
            }
            else if (Horizontal_Change > 0.0f)
            {
                transform.localScale = new Vector3(1.0f, 1.0f, 1.0f);
            }
            Animator.SetBool("running", Horizontal_Change != 0.0f);
            if (UnityEngine.Input.GetKey(KeyCode.LeftShift) && Time.time > LastShoot + 0.25f)
            {
